...
if (!dbUri || !customerEmail || !supplierEmail) throw new Error('R2 concurrency environment incomplete')
...
;(Payment.prototype as any).create = async ({ body }: any) => {
  ...
  // Keep the first provider call open briefly so a competing request has time
  // to cross any non-atomic preflight check. This does not alter application DB state.
  await new Promise((resolve) => setTimeout(resolve, 250))
  const id = `r2-race-${runId}-${++providerSequence}`
  const payment = {
    id,
    status: 'pending',
...
try {
  ...
  if (checkout.response.status !== 200 || !(checkout.body as any)?.bookingId) {
    throw new Error(`Race booking failed HTTP ${checkout.response.status}`)
  }
  const bookingId = String((checkout.body as any).bookingId)

  const quote = await request(`/api/mercadopago/quote/${bookingId}/${sessionId}`)
  if (quote.response.status !== 200) throw new Error(`Race quote failed HTTP ${quote.response.status}`)

  const paymentRequest = (key: string) => request('/api/create-mercadopago-payment', {
    method: 'POST',
    headers: {
  ...
  const [first, second] = await Promise.all([
    paymentRequest(`r2-race-a-${runId}`),
    paymentRequest(`r2-race-b-${runId}`),
  ])
  ...
  evidence.activeTransactions = await PaymentTransaction.countDocuments({
    booking: new mongoose.Types.ObjectId(bookingId),
    status: { $in: [PaymentStatus.Pending, PaymentStatus.Approved] },
  })
  const reservation = await ReservationState.findOne({ booking: new mongoose.Types.ObjectId(bookingId) }).lean()
  evidence.reservationStatus = String(reservation?.status || '')

  const accepted = evidence.responses.filter((item) => item.status === 200 || item.status === 201).length
  const blocked = evidence.responses.filter((item) => item.status === 409).length
  ...
  if (
    ...
    evidence.failure = `Concurrent payment race reproduced: providerCreates=${providerCreateCalls}, activeTransactions=${evidence.activeTransactions}, accepted=${accepted}, blocked=${blocked}`
    process.exitCode = 1
  } else {
  ...
  await fs.mkdir(evidenceDir, { recursive: true })
  await fs.writeFile(path.join(evidenceDir, 'mitos-r2a-concurrency.json'), `${JSON.stringify(evidence, null, 2)}\n`, 'utf8')
  await mongoose.disconnect().catch(() => undefined)
  console.log(`[R2A-race] RESULT=${evidence.result} ${evidence.failure}`)
}
