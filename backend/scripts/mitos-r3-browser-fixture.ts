...
if (!dbUri || !supplierEmail) {
  throw new Error('R3 browser fixture requires BC_DB_URI and MITOS_DEMO_SUPPLIER_EMAIL')
}
...
try {
  ...
  if (!supplier) throw new Error('R3 seeded supplier was not found')
  ...
  if (!car) throw new Error('R3 seeded Toyota Raize was not found')

  const location = await Location.findOne({ supplier: supplier._id }).lean()
  if (!location) throw new Error('R3 seeded location was not found')
  ...
  console.log(`[R3] browser fixture descriptor written: ${fixturePath}`)
} finally {
