...
const CarCheckin = () => {
                ...
                if (id) {
                    ...
                    try {
                        ...
                        if (_booking) {
                            ...
                            setSelectedCar({ name: car.name })
                            setDriver({ name: (_booking.driver as bookcarsTypes.User).fullName || '' })
                        }
                    } catch (err) {
                        console.error(err)
                    }
        ...
        if (step < STEPS.length - 1) {
            ...
            try {
                ...
                if (missingPhotos.length > 0) {
                    formData.append('remarksIn', `Omisión de imágenes bajo responsabilidad: ${missingPhotos.join(', ')}`)
                }
    ...
    const S: any = {
        ...
        title: { fontSize: 26, fontWeight: 700, color: '#2e7d32', margin: 0 },
        ...
        btnPrimary: { width: '100%', padding: '16px', background: '#2e7d32', color: '#fff', border: 'none', borderRadius: 8, fontSize: 16, fontWeight: 700, cursor: 'pointer', marginTop: 8 },
            ...
            <Layout onLoad={onLoad} strict>
                <div style={S.wrap}>
                    <div style={{ ...S.card, margin: '60px 20px', textAlign: 'center', maxWidth: 440 }}>
                        ...
                        <h2 style={{ fontSize: 24, color: '#212121' }}>¡Ingreso registrado!</h2>
                        <p style={{ color: '#757575' }}>El check-in del vehículo se completó con éxito.</p>
    ...
    return (
        <Layout onLoad={onLoad} strict>
            {!loading && (
                <div style={S.wrap}>
                    <div style={S.header}>
                        <h1 style={S.title}>Recepción de Vehículo</h1>
                    </div>
                            ...
                            <div key={s} style={{ display: 'flex', alignItems: 'center' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, minWidth: 60 }}>
                                    <div style={{
                                        ...
                                        background: i < step ? '#2e7d32' : i === step ? '#e8f5e9' : '#e0e0e0',
                                        color: i < step ? '#fff' : i === step ? '#2e7d32' : '#9e9e9e',
                                        fontSize: 12, fontWeight: 700, border: i === step ? '2px solid #2e7d32' : 'none',
                                    }}>{i < step ? '✓' : i + 1}</div>
                                    <span style={{ fontSize: 10, color: i === step ? '#2e7d32' : '#9e9e9e' }}>{s}</span>
                                </div>
                                {i < STEPS.length - 1 && <div style={{ width: 40, height: 2, background: i < step ? '#2e7d32' : '#e0e0e0', marginBottom: 16 }} />}
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={S.card}>
                        ...
                        {step === 2 && (
                            <>
                                <h2 style={{ fontSize: 20 }}>Confirmar Recepción</h2>
                                {[
