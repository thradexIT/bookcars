  ...
  return (
    <View>
        ...
        {CAROUSEL_SLOTS.map((_, i) => (
          <TouchableOpacity
            ...
            style={[
              ...
              photos[i] ? pc.dotFilled : i === activeIndex ? { backgroundColor: accentColor } : pc.dotDefault,
            ]}
          />
        ))}
      </View>
      ...
      <View style={pc.nav}>
        ...
        <TouchableOpacity
          style={[pc.navBtn, { backgroundColor: activeIndex === total - 1 ? '#e8f5e9' : accentColor, borderColor: accentColor }]}
          onPress={() => setActiveIndex(Math.min(total - 1, activeIndex + 1))}
          disabled={activeIndex === total - 1}
        >
          <Text style={[pc.navBtnText, { color: activeIndex === total - 1 ? '#a5d6a7' : '#fff' }]}>Siguiente</Text>
          <ChevronRight size={18} color={activeIndex === total - 1 ? '#a5d6a7' : '#fff'} />
        </TouchableOpacity>
      </View>
      <ValidatedCamera
        visible={cameraVisible}
        onClose={() => setCameraVisible(false)}
        onCapture={(uri) => { setPhotoAtIndex(cameraTarget, uri); setCameraVisible(false) }}
...
const KmSlot = ({
  ...
  accentColor: string
  title: string
  capturedBorderColor?: string
}) => {
  const [cameraVisible, setCameraVisible] = useState(false)
  const borderC = capturedBorderColor ?? accentColor
  ...
  return (
    <View style={km.container}>
      ...
      <View style={sl.pair}>
        ...
        <TouchableOpacity
          style={[sl.box, sl.captureBox, photoUri && { borderStyle: 'solid', borderColor: borderC, backgroundColor: '#e8f5e9' }]}
...
export default function CheckinScreen() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      formData.append('kmIn', mileage)
      formData.append('fuelIn', fuel)

      const missing = CAROUSEL_SLOTS.filter((_, i) => !carPhotos[i]).map(s => s.label)
      if (missing.length > 0) {
        formData.append('remarksIn', `Omisión de imágenes bajo responsabilidad: ${missing.join(', ')}`)
      }
      if (kmPhoto) {
      ...
      await BookingService.checkinReturn(bookingId, formData)
      setDone(true)
    } catch (err: any) {
      Alert.alert('Error', err?.message ?? 'No se pudo registrar la recepción.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.doneWrap}>
          <CheckCircle color={ACCENT} size={72} />
          <Text style={styles.doneTitle}>¡Ingreso registrado!</Text>
          <Text style={styles.doneSubtitle}>El check-in del vehículo se completó con éxito.</Text>
          <TouchableOpacity style={[styles.doneBtn, { backgroundColor: ACCENT }]} onPress={() => router.replace('/')}>
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <Stack.Screen options={{ title: 'Recepción de Vehículo', headerShown: true }} />
  ...
  return (
    <View style={styles.container}>
      <Stack.Screen options={{
        title: plate ? `Recepción: ${plate}` : 'Recepción de Vehículo',
        headerShown: true,
        headerLeft: () => (
          ...
          <View key={s} style={styles.stepItemWrapper}>
            <View style={styles.stepItem}>
              <View style={[styles.stepCircle,
                ...
                i === step ? { backgroundColor: '#e8f5e9', borderWidth: 2, borderColor: ACCENT } :
                styles.stepInactive]}>
                {i < step
                  ? <Check color="#fff" size={14} />
                  : <Text style={[styles.stepNumber, i === step && { color: ACCENT }]}>{i + 1}</Text>}
              </View>
              <Text style={[styles.stepLabel, i === step && { color: ACCENT }]}>{s}</Text>
            </View>
            {i < STEPS.length - 1 && (
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {step === 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Nivel de Combustible (Entrada)</Text>
            <View style={styles.fuelOptions}>
              {[
                ...
                <TouchableOpacity
                  key={val}
                  style={[styles.fuelBtn, fuel === val && { backgroundColor: ACCENT, borderColor: ACCENT }]}
                  onPress={() => setFuel(val)}
                >
            ...
            <KmSlot
              ...
              accentColor={ACCENT}
              title="Tablero — Kilometraje de Entrada"
              capturedBorderColor={ACCENT}
            />
          </View>
        )}
        ...
        {step === 2 && (
          <View style={styles.card}>
            ...
            {[
              ...
              { label: 'Km de entrada', value: mileage ? `${mileage} km` : '-' },
            ...
            {carPhotos.some(f => !f) && (
              <View style={styles.responsibilityBox}>
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <View style={[styles.checkbox, acceptedResponsibility && { backgroundColor: ACCENT, borderColor: ACCENT }]}>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[styles.nextBtn, { backgroundColor: ACCENT }, (!canNext() || submitting) && styles.nextBtnDisabled]}
          onPress={handleNext}
        ...
        >
          {submitting
            ...
            : <Text style={styles.nextBtnText}>
                {step === STEPS.length - 1 ? '✓ Aprobar Ingreso' : 'Continuar →'}
              </Text>}
        </TouchableOpacity>
      </View>
    </View>
...
// ─────────────────────────────────────────────────────────────────
//  SHARED STYLES (also used by sub-components via module scope)
// ─────────────────────────────────────────────────────────────────
...
const sl = StyleSheet.create({
  ...
  capturedBox:     { borderStyle: 'solid', borderColor: '#2e7d32', backgroundColor: '#e8f5e9' },
  boxTag:          { position: 'absolute', top: 5, left: 6, fontSize: 9, fontWeight: '700', color: '#2e7d32', backgroundColor: 'rgba(255,255,255,0.85)', paddingHorizontal: 4, borderRadius: 4, zIndex: 1 },
...
const styles = StyleSheet.create({
  ...
  nextBtn:         { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 16, borderRadius: 10, backgroundColor: '#2e7d32' },
  ...
  doneBtn:         { marginTop: 8, paddingHorizontal: 32, paddingVertical: 14, borderRadius: 10 },
